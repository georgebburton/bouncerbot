...
# Optional: YouTube cookies to avoid "Sign in to confirm you're not a bot" on servers.
# Use COOKIES_FILE (path to file) or COOKIES_TXT (base64-encoded file content, e.g. for Railway).
_cookiefile = None
_cookies_path = os.getenv("COOKIES_FILE") or os.getenv("YTDL_COOKIES_FILE")
if _cookies_path and os.path.isfile(_cookies_path):
    _cookiefile = os.path.abspath(_cookies_path)
else:
    _cookies_b64 = os.getenv("COOKIES_TXT")
    if _cookies_b64:
        try:
            _raw = base64.b64decode(_cookies_b64, validate=True).decode("utf-8", errors="replace")
            _fd, _cookiefile = tempfile.mkstemp(suffix=".txt", prefix="ytdl_cookies_")
            os.write(_fd, _raw.encode("utf-8"))
            os.close(_fd)
        except Exception:
            _cookiefile = None
